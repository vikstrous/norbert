...
trait PartitionedLoadBalancer[PartitionedId] {
   ...
   */
  def nodesForOneReplica(id: PartitionedId, capability: Option[Long] = None, permanentCapability: Option[Long]): Map[Node, Set[Int]]
   ...
   */
  def nodesForPartitionedId(id: PartitionedId, capability: Option[Long] = None, permanentCapability: Option[Long]): Set[Node]
   ...
   * @return the <code>Nodes</code> to broadcast the next message to a replica to
   */
  def nodesForPartitions(id: PartitionedId, partitions: Set[Int], capability: Option[Long] = None, permanentCapability: Option[Long] = None): Map[Node, Set[Int]]
   ...
   * @return
   */
  def nodesForPartitionedIds(ids: Set[PartitionedId], capability: Option[Long] = None, permanentCapability: Option[Long]): Map[Node,  Set[PartitionedId]]  =
  {
    ids.foldLeft(Map[Node, Set[PartitionedId]]().withDefaultValue(Set())) { (map, id) =>
      val node = nextNode(id, capability, permanentCapability).getOrElse(throw new NoNodesAvailableException("Unable to satisfy request, no node available for id %s".format(id)))
